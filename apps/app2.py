...
def load_pallet_detail_df():
    ...
    - 托盘重量：仅来自托盘明细，按托盘求和
    - 托盘体积（CBM）：由 L/W/H(inch) 计算（每个托盘仅计算一次体积，避免多行重复累加）
    ...
    if "托盘号" not in df.columns:
        for cand in ["托盘ID","托盘编号","PalletID","PalletNo"]:
            if cand in df.columns:
                df = df.rename(columns={cand:"托盘号"}); break
    if "托盘号" not in df.columns: df["托盘号"] = pd.NA
    ...
    if weight_col is None:
        df["托盘重量"] = pd.NA; weight_col = "托盘重量"
    df[weight_col] = pd.to_numeric(df[weight_col], errors="coerce")

    # —— 识别长/宽/高（inch），每个托盘仅计算一次体积（取该托盘下“第一组非空 L/W/H”）
    ...
    # 分组到“托盘号+仓库代码”后：
    # - 重量：求和
    # - 体积：从该组内取第一组非空 L/W/H（均>0）计算一次 L*W*H*(0.0254^3)
    # - 运单：聚合列表
    def _agg_group(g):
        p_wt = pd.to_numeric(g[weight_col], errors="coerce").dropna().sum()
        # 取第一组非空 L/W/H
        L = _first_valid(g[len_col]) if len_col in g.columns else None
        W = _first_valid(g[wid_col]) if wid_col in g.columns else None
        H = _first_valid(g[hei_col]) if hei_col in g.columns else None
        cbm = None
        if all(x is not None and x > 0 for x in [L, W, H]):
            cbm = (L * W * H) * (INCH_TO_M ** 3)

        wbs = [x for x in g["运单号"] if pd.notna(x) and str(x).strip()!='']
        return pd.Series({
            "托盘重量": float(p_wt) if pd.notna(p_wt) else None,
            "托盘体积": float(cbm) if cbm is not None else None,
            "运单清单_list": wbs
        })
    ...
    for _, brow in base.iterrows():
        ...
        if diffs_days:
            def keyfn(s):
                try: a, _ = s.split("-", 1); return int(a)
                except: return 99999
...
def build_waybill_delta():
    ...
    for _, r in track.iterrows():
        for wb in _split_waybill_list(r.get("运单清单", "")):
            wb = _norm_waybill_str(wb)
            if wb:
    ...
    weight_map = dict(zip(arrivals["运单号"], pd.to_numeric(arrivals["收费重"], errors="coerce")))

    for _, r in track.iterrows():
        waybills_raw = _split_waybill_list(r.get("运单清单",""))
        waybills = [wb for wb in waybills_raw if wb in wb_from_track]
        if not waybills:
            continue
    ...
    if not arrivals.empty:
        arr2 = arrivals[["运单号","ETD/ATD","ETA/ATA"]].copy()
        arr2 = arr2.rename(columns={"ETD/ATD":"发出(ETD/ATD)", "ETA/ATA":"到港(ETA/ATA)"})
        out = out.merge(arr2, on="运单号", how="left")
    else:
        out["发出(ETD/ATD)"] = pd.NA
        out["到港(ETD/ATD)"] = pd.NA

    cust_bol = bol[["运单号","客户单号"]] if ("客户单号" in bol.columns) else pd.DataFrame(columns=["运单号","客户单号"])
    ...
    if not cust_all.empty:
        ...
        cust_all = cust_all.sort_values(["运单号","_pri"]).drop_duplicates(subset=["运单号"], keep="first")[["运单号","客户单号"]]
...
st.markdown("""
**分摊比例计算公式：**  
每个托盘的分摊比例 = 该托盘重量 ÷ 所有选中托盘重量总和  

**分摊费用计算公式：**  
每个托盘的分摊费用 = 分摊比例 × 本车总费用  
（最后一托盘会自动调整几分钱的差额，确保总额严格等于本车总费用）
""")
